...
(function($) {
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			...
			/*
			 * Variable: doTotal
			 * Purpose:  Do u want real time Total
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.doTotal = false;
		}
		...
		function _fnDraw( oSettings )
		{
			...
			/*--------custome added for live search--------*/
				//alert(oSettings.showTotal.length);
				if(oSettings.doTotal == true && oSettings.totalColIndx != "" && oSettings.showTotal != "")
					_fngetTotal( oSettings );
		}
		
		
		function _fngetTotal( oSettings )
		{			
			var iclLength = oSettings.showTotal.length;
			var rows = $("#"+oSettings.sTableId+" tr:gt(0)");
			
			for(icl=0; icl<iclLength; icl++)
			{
				var sumTotal = 0;
				rows.children("td:nth-child("+oSettings.totalColIndx[icl]+")").each(function() {
					if(!isNaN($(this).html()))
					{
						oneTemp = Number($(this).html());
						sumTotal = (sumTotal * 1) + (oneTemp * 1);
					}
				});
				$("#"+oSettings.showTotal[icl]).html(sumTotal);
			}
		}
		...
		function _fnAddOptionsHtml ( oSettings )
		{
			...
			var sDom = oSettings.sDom.replace( "H", "fg-toolbar ui-widget-header  ui-helper-clearfix" );
		...
		function _fnFeatureHtmlLength ( oSettings )
		{
            ...
            {
               sStdMenu = sStdMenu + '<option value="'+x+'">'+x+'</option>';
            }
			sStdMenu = sStdMenu + '</select>';
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "doTotal" );
				_fnMap( oSettings, oInit, "totalColIndx" );
				_fnMap( oSettings, oInit, "showTotal" );
...
function getTotal(tableId, columnIndx, displayPositionId)
{
    var sumTotal = 0;
	var rows = $("#"+tableId+" tr:gt(0)");		
	rows.children("td:nth-child("+columnIndx+")").each(function() {
        if(!isNaN($(this).html()))
        {
    	   oneTemp = Number($(this).html());
    	   sumTotal = (sumTotal * 1) + (oneTemp * 1);
	...
	$("#"+displayPositionId).html(Math.round(sumTotal));
}
...
$.fn.dataTableExt.oApi.fnReloadAjax = function ( oSettings, sNewSource, fnCallback, bStandingRedraw )
{
	...
	oSettings.fnServerData( oSettings.sAjaxSource, [], function(json) {console.log()
